...
void Ui::PlayerHp()
{
	...
	if (S_Ui.PlayerHp != S_Ui.PlayerHpMemory)
	{
		...
		{
			S_UiTexture.PlayerHpTexture.SetMulColor({ 0.0f,1.0f,0.0f,1.0f });//—Î
		}else {
			if (S_Ui.PlayerHp >= 0.33 * 100)
			{
				S_UiTexture.PlayerHpTexture.SetMulColor({ 1.0f,1.0f,0.0f,1.0f });//‰©
			}else {
				if (S_Ui.PlayerHp >= 0.0 * 100)
				{
					S_UiTexture.PlayerHpTexture.SetMulColor({ 1.0f,0.0f,0.0f,1.0f });//Ô
				}
			}
		}
	}
