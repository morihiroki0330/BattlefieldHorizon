...
namespace nsK2EngineLow {
	void Level2DRender::AddMapChip2DRender(Level2DRenderObjectData* objData)
	{
	...
	void Level2DRender::Init(
		const char* filePath,
		std::function<bool(Level2DRenderObjectData& objData)> hookFunc
	)
	{
		//Caslファイルをロードする。
		CaslFile casl;
		...
		for (int i = 0; i < casl.GetNumCaslData(); i++)
		{
			...
			Level2DRenderObjectData objData;
